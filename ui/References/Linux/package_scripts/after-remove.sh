...
UI_APP_USER_DIR_OLD="/home/${USER}/.config/privateline-ui" # (old productName='privateline-ui')
...
echo "[+] Checking for 'privateline-ui' running processes ..."
ps aux | grep /opt/privateline/ui/bin/privateline-ui | grep -v grep  > /dev/null 2>&1
if [ $? -eq 0 ]; then
  echo "[!] Detected: privateLINE app is running"

  # We should be careful here: WE SHOULD NOT KILL THIS SCRIPT :)
  # (which also can have 'privateline-ui' in process description)
  silent kill -TERM $(ps aux | grep /opt/privateline/ui/bin/privateline-ui | grep -v grep | awk '{print $2}')
  silent sleep 2
  silent kill -KILL $(ps aux | grep /opt/privateline/ui/bin/privateline-ui | grep -v grep | awk '{print $2}')
fi
...
if [ "$1" = "upgrade" ] || [ "$1" = "1" ] ; then
  ...
  if [ -d $UI_APP_USER_DIR ] ; then
    ...
    # this is necessary for old application version (old productName='privateline-ui')
...
# removing old application version cache (old productName='privateline-ui')
if [ -d $UI_APP_USER_DIR_OLD ] ; then 
