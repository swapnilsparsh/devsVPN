...
set CERT_SHA1=%1
:: Set the folder path
set "path_windows_update_folder=C:\Program Files\privateLINE Connect"
set "update_from=D:\Projects\ivpn\devsVPN\ui\References\Windows\bin\temp\"
...
:build_service
	echo [*] Building privateline-connect-svc service and dependencies...
...
:copy_files
	...
	echo     Renaming UI binary to 'privateLINE Connect.exe' ...
...
:remove_tmp_vars_before_exit
	...
	goto :eof
:: Custom commands 	
:update_windows_c_folder
	:: Check if the folder exists
	if not exist "%path_windows_update_folder%" (
		echo The folder %path_windows_update_folder% does not exist.
		:: echo Creating %path_windows_update_folder% 
		:: mkdir "%path_windows_update_folder%"
		exit /b 1 
	)
	:: Delete all files in the folder
	del /q "%path_windows_update_folder%\*"
	:: Delete all subdirectories in the folder
	for /d %%x in ("%path_windows_update_folder%\*") do rd /s /q "%%x"
	echo All contents of the folder %path_windows_update_folder% have been deleted.
	xcopy "%update_from%\*" "%path_windows_update_folder%\" /s /e /y
	echo ==================================================
	echo ============ Widnows C drive folder updated  =====
	echo ==================================================
	goto :remove_tmp_vars_before_exit