...
set RELEASE_PATH=%SCRIPTDIR%privateline-split-tunnel\x64\Release\privateline-split-tunnel
...
echo [+] Building ...
msbuild "%SCRIPTDIR%privateline-split-tunnel\privateline-split-tunnel.vcxproj" /p:Configuration=Release /p:Platform=x64 /p:SignMode=Off || goto :error
...
copy "%RELEASE_PATH%\privateline-split-tunnel.inf" "%DIST_PATH_DRV%\privateline-split-tunnel.inf" || goto :error
copy "%RELEASE_PATH%\privateline-split-tunnel.sys" "%DIST_PATH_DRV%\privateline-split-tunnel.sys" || goto :error
copy "%RELEASE_PATH%\WdfCoinstaller01009.dll" "%DIST_PATH_DRV%\WdfCoinstaller01009.dll" || goto :error
copy "%RELEASE_PATH%\..\privateline-split-tunnel.pdb" "%DIST_PATH_DRV%\privateline-split-tunnel.pdb" || goto :error

rem echo [+] Signing SYS file by EV Certificate ...
rem set TIMESTAMP_SERVER=http://timestamp.digicert.com
rem signtool sign /tr %TIMESTAMP_SERVER% /td sha256 /fd sha256 /sha1 %CERT_SHA1% /v "%DIST_PATH_DRV%\privateline-split-tunnel.sys" || goto :error

echo [+] Preparing CAB file configuration...
>"%DIST_PATH_OUT%\privateline-split-tunnel.ddf" (
  ...
  echo .Set CabinetNameTemplate=privateline-split-tunnel.cab
  echo .Set DestinationDir=Package
  echo .Set DiskDirectoryTemplate="%DIST_PATH_OUT%"
  echo "%DIST_PATH_DRV%\privateline-split-tunnel.inf"
  echo "%DIST_PATH_DRV%\privateline-split-tunnel.sys"
  echo "%DIST_PATH_DRV%\WdfCoinstaller01009.dll"
	echo "%DIST_PATH_DRV%\privateline-split-tunnel.pdb"
)
IF %ERRORLEVEL% NEQ 0 goto error

echo [+] Generating CAB file ...
cd "%DIST_PATH_OUT%"
makecab /f "%DIST_PATH_OUT%\privateline-split-tunnel.ddf" || goto :error
