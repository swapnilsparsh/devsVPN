...
func ApplyConfig(isStEnabled, isStInverse, enableAppWhitelist, isStInverseAllowWhenNoVpn, isVpnEnabled bool, addrConfig ConfigAddresses, splitTunnelApps []string) (retErr error) {
	mutex.Lock()
	defer func() {
		log.Info("ApplyConfig() completed")
		mutex.Unlock()
	}()
	log.Info(fmt.Sprintf("ApplyConfig() started: isStEnabled=%t, enableAppWhitelist=%t, isVpnEnabled=%t", isStEnabled, enableAppWhitelist, isVpnEnabled))
